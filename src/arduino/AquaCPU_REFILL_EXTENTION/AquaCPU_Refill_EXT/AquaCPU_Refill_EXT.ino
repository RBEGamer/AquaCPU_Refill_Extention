...
char msgString[128];
#define CAN_BUS_ID_GENERAL_ERR 0x42
#define CAN_BUS_ID_GENERAL_STOP 0x43
#define CAN_BUS_ID_REFILL_STARTED 0x8
#define CAN_BUS_ID_REFILL_STOP 0x9
#define CAN_BUS_ID_MANUAL_REFILL 0x10
#define CAN_BUS_ID_REFILL_FINISH 0x11
#define CAN_BUS_TANK_STATE 0x12
...
void send_can_message(int _id, float _val) {
  ...
  byte arr[4] = { 0 };
  floatToByte(arr, _val);
  byte sndStat = CAN0.sendMsgBuf(_id, 0, 4, arr);
  if (sndStat == CAN_OK) {
...
send_can_message(CAN_BUS_ID_MANUAL_REFILL, true);
  ...
  if (valve_state) {
    ...
    lcd.setCursor(0, 1);
    unsigned long tr = (AUTO_OFF_TIMER_INTERVALL - ( millis() - lastmillis) / 1000);
    lcd.print("IN MAX:" + String(tr));
   //send_can_message(CAN_BUS_ID_REFILL_UPDATE_TIME_REMEANING, tr);
    delay(2000);
#endif
